  ...
  while (true) {
    try {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        int t, m, p;
        ss1 >> l >> n >> t >> m >> p;
        if (l == "Player" && findPlayer(n) != nullptr && t >= 0 && t <= 4 && m >= 0 && p >= 0) {
          player[i]->name = n;
          player[i]->cup = t;
          player[i]->money = m;
          player[i]->setPos(p);
        } else { throw ios::failbit; }
        if (p = 10) {
          int b;
          ss1 >> b;
          if (b == 0) {
            player[i]->isInTimLine = false;
          } else if (b == 1) {
            player[i]->isInTimLine = true;
            int nt;
            ss1 >> nt;
            if(nt == 0 || nt == 1 || nt == 2) {
              player[i]->TimLineTurn = nt;
            } else { throw ios::failbit; }
          } else {
            throw ios::failbit;
          }  
      ...
      curPlayer = findPlayer(player[1]->name);
      ...
      for (int i = 0; i < 40; ++i) {
        ...
        int im;
        ss2 >> bn >> o >> im;
        if (findBuilding(bn) != nullptr && (findPlayer(o) != nullptr || o == "BANK") && im >= -1 && im <= 5) {
          if(findBuilding(bn)->getBuildingType() != BuildingType::Nonproperty) {
            Property *prop = static_cast<Property *>(findBuilding(bn));
            if(o == "BANK") {
              prop->owner = nullptr;
            } else {
              prop->setowner(findPlayer(o));   
            }
            if(im == -1) {
              prop->mtg = true;
            } else {
            ...
            if(findBuilding(bn)->getBuildingType() == BuildingType::Academic) {
              findBuilding(bn) -> improvement = im;
            }  
          }
        } else { throw ios::failbit; }
      }
        ...
        cout << "Invalid file input, please check the file." << endl;
        infile.clear();
        infile.ignore();
        return false;
    }
